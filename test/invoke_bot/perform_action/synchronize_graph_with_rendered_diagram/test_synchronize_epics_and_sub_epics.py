...
def _create_epic_xml(helper, sub_epic_names=None, epic_name='Payment Processing'):
    ...
    for idx, se_name in enumerate(sub_epic_names):
        # Use hierarchical cell IDs for tool-generated appearance (enables rename detection)
        se_cell_id = f'epic-1/sub-epic-{idx+1}'
        se_cells.append(
            f'<mxCell id="{se_cell_id}" value="{se_name}" '
        ...
        for sidx, story_name in enumerate(stories):
            story_cell_id = f'epic-1/sub-epic-{idx+1}/story-{sidx+1}'
            story_cells.append(
                f'<mxCell id="{story_cell_id}" value="{story_name}" '
