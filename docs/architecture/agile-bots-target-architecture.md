# Context Engine Target Architecture — TypeScript/Node-Centric
...
| **Client → domain + DOM** | Webview loads shared domain (bundled); adds DOM only. Shared logic; single source. |
...
└─────────────────────────────────────────────────────────────────────────────┬┘
            ...
            ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│  DOMAIN (Pure TS — Engine → Counter, Foo)                                     │
│  Shared by client and CLI.                                                    │
└─────────────────────────────────────────────────────────────────────────────┬┘
                                                                              │
                                                                              ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
### Domain
Engine (root) loads Counter; Counter has Foo. Pure TS. **All counter implementations share `ICounter`** so domain, server domain, server view, client DOM adapter, and CLI adapters are interchangeable at the interface level.
...
const engine = new Engine();  // CLI uses plain Counter (in-memory)
...
| **EngineView** | Extends BaseView; loads Engine.html; owns Engine; delegates content to sub-views; handles postMessage. Delegates child markup to sub-views. |
| **CounterView** | Extends BaseView; loads Counter.html; delegates to domain (CounterServer); posts to webview. Owns counter markup. |
| **engine_client.ts** | Client: uses **shared Counter** (bundled); DOM only (`updateDom`); `syncToServer(command, value)`. Shared logic; DOM binds to domain state. |
...
EngineView extends BaseView. It owns Engine and sub-views. It delegates HTML to each view. Commands use paths like `counter.count`; lookup delegates to sub-sections.
...
export class EngineView extends BaseView {
  ...
  private _getHtml(): string {
    ...
    const counterHtml = this.counter.getHtml();  // delegate to sub-view
...
CounterView extends BaseView and **implements ICounter**. It receives the panel, counter (from Engine), and extensionUri. It owns its HTML template as a property; `getHtml()` uses that property. Methods delegate to domain and post to webview.
...
export class CounterView extends BaseView implements ICounter {
  ...
  getHtml(): string {
    ...
    return this.renderTemplateContent(CounterView.template, {
...
**Note:** `domCounter` implements `ICounter`; same pattern as server view—each method updates only the DOM element that changes. Shared logic; DOM binds to domain state.
...
│
└── counter/
    ├── counter.ts          # Domain (Counter + Foo in same file; pure TS)
...
- **Client:** Domain (bundled) + DOM adapter. Webview loads shared Counter; shared logic.
