...
<a id="scenario-panel-applies-ace-branding-when-config-specifies-ace-brand"></a>
### Scenario: [Panel applies ACE branding when config specifies ACE brand](#scenario-panel-applies-ace-branding-when-config-specifies-ace-brand) (happy)

**Steps:**
```gherkin
GIVEN: Config file exists at conf/config.json with branding property set to 'ACE'
WHEN: Panel initializes with workspace root
THEN: Branding module reads conf/config.json
