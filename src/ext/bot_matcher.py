...
class BotMatcher:
    ...
    def _load_bot_registry(self) -> Dict[str, Dict]:
        registry_path = self.bot_paths.python_workspace_root / 'bots' / 'registry.json'
        try:
    ...
    def _load_bot_triggers(self) -> BotTriggers:
        trigger_file = self.bot_paths.python_workspace_root / 'bots' / self.bot_name / 'trigger_words.json'
