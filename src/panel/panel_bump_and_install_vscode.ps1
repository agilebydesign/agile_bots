# Bump version, rebuild, and reinstall the Bot Panel extension (VS Code)
# Usage: .\panel_bump_and_install_vscode.ps1 [patch|minor|major] [-NoReload]
...
param(
    ...
    # If multiple versions accumulated in the extensions directory, purge them.
    # This avoids VS Code loading an older version from disk.
    [switch]$PurgeAllVersions = $true
)
...
# Prefer the real VS Code CLI (avoid Cursor's code.cmd shim on PATH)
$vscodeCode = Join-Path $env:LOCALAPPDATA "Programs\Microsoft VS Code\bin\code.cmd"
$codeExe = "code"
if (Test-Path $vscodeCode) {
    $codeExe = $vscodeCode
} else {
    Write-Host "Warning: Could not find VS Code CLI at: $vscodeCode" -ForegroundColor Yellow
    Write-Host "Falling back to 'code' on PATH (may target Cursor)." -ForegroundColor Yellow
}

# Warn if 'code' on PATH is Cursor's shim (common source of confusion)
try {
    $codeCmd = Get-Command code -ErrorAction SilentlyContinue
    if ($codeCmd -and $codeCmd.Path -and ($codeCmd.Path -like '*\\cursor\\resources\\app\\codeBin\\code.cmd')) {
        Write-Host "Warning: 'code' on PATH appears to be Cursor's CLI: $($codeCmd.Path)" -ForegroundColor Yellow
        Write-Host "This script will use: $codeExe" -ForegroundColor Yellow
    }
} catch {
...
& $codeExe --uninstall-extension agilebot.bot-panel > $null
if ($LASTEXITCODE -ne 0) {
...
# Purge any leftover folders for this extension (prevents older copies being loaded)
if ($PurgeAllVersions) {
    Write-Host "      Purging on-disk extension folders (agilebot.bot-panel-*)..." -ForegroundColor Cyan
    $extRoots = @(
    ...
    foreach ($root in $extRoots) {
        if (Test-Path $root) {
            Get-ChildItem $root -Directory -ErrorAction SilentlyContinue |
                Where-Object { $_.Name -like 'agilebot.bot-panel-*' } |
                ForEach-Object {
                    try {
    ...
    Write-Host "      Done: Old folders purged" -ForegroundColor Green
}
...
& $codeExe --install-extension "$vsixPath" --force > $null
if ($LASTEXITCODE -ne 0) {
...
try {
    $installed = & $codeExe --list-extensions --show-versions 2>$null | Where-Object { $_ -like 'agilebot.bot-panel*' } | Select-Object -First 1
    if ($installed) {
        Write-Host "      Installed: $installed" -ForegroundColor Green
    } else {
        Write-Host "      Warning: Could not confirm via CLI output (but install succeeded)." -ForegroundColor Yellow
    }
} catch {
    Write-Host "      Warning: Version verification failed: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
if (-not $NoReload) {
    ...
    try {
        # Trigger reload via VS Code command URI scheme
        $reloadCmd = "vscode://command/workbench.action.reloadWindow"
    ...
    Write-Host ""
    Write-Host "To skip auto-reload: .\panel_bump_and_install_vscode.ps1 -NoReload" -ForegroundColor DarkGray
}
Write-Host ""
