...
class BehaviorsView extends PanelView {
    ...
    renderAction(action, bIdx, aIdx, behaviorName, behaviorNameJs, behaviorNameRaw, plusIconPath, subtractIconPath, tickIconPath, notTickedIconPath, combinedIconPath, skipIconPath, manualIconPath, executionSettings = {}, specialInstructions = {}) {
        ...
        const actionToggleGroup = `<span class="execution-toggle-container" id="${actionExecToggleId}" style="flex-shrink: 0;" onclick="event.stopPropagation();">${actionCollapsedBtn}${actionExpandedGroup}</span>`;
        ...
        const actionHtml = `<div class="collapsible-header action-item card-item${actionActiveClass}${actionSkipClass}" data-skip="${isSkipAction}" title="${actionTooltip}" style="display: flex; align-items: center; justify-content: space-between; flex-wrap: nowrap; gap: 4px;">
