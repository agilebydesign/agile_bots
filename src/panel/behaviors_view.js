...
class BehaviorsView extends PanelView {
    ...
    renderAction(action, bIdx, aIdx, behaviorName, behaviorNameJs, behaviorNameRaw, plusIconPath, subtractIconPath, tickIconPath, notTickedIconPath, combinedIconPath, skipIconPath, manualIconPath, executionSettings = {}) {
        ...
        const actionCollapsedBtn = currentModeObj.iconPath ? `<button class="execution-toggle-btn active execution-toggle-collapsed" data-action="toggleExecutionToggle" data-target="${actionExecToggleId}" title="Configure ${actionName} execution"><img src="${currentModeObj.iconPath}" alt="${currentModeObj.tooltip}" style="width: 22px; height: 22px; object-fit: contain; display: block;" /></button>` : '';
