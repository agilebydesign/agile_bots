/**
 * WorkspaceSectionView - Renders Diagrams and Build sections between story map and instructions.
 */
...
class WorkspaceSectionView extends PanelView {
    ...
    async render() {
        ...
        const drawioLink = (botData?.scope?.graphLinks || []).find(l => l.url && l.url.endsWith('.drawio'));
        const drawioPath = drawioLink ? escapeForJs(drawioLink.url) : '';

        const diagramsHtml = this._renderDiagramsSubsection(instructionsData, currentAction, renderDiagramIconPath, saveLayoutIconPath, clearLayoutIconPath, generateReportIconPath, updateGraphIconPath, drawioPath);
        const buildHtml = this._renderBuildSubsection(instructionsData, currentAction, botData, jsonIconPath, filesIconPath);
    ...
    _renderDiagramsSubsection(instructions, currentAction, renderIcon, saveIcon, clearIcon, reportIcon, updateIcon, drawioPath) {
        let content = '';

        // Diagram action buttons
        content += `
            <div id="ws-diagram-buttons" style="display: flex; align-items: center; gap: 2px; margin-bottom: 8px;">
        ...
        // Diagram files (from diagrams array)
        if (instructions.diagrams && instructions.diagrams.length > 0) {
            const diagramView = new DiagramSectionView(instructions.diagrams);
            content += diagramView.renderSection();
        }

        // Render output links
        const outputPaths = instructions.render_output_paths || [];
        if (outputPaths.length > 0) {
            content += '<div style="margin-top: 6px;">';
            outputPaths.forEach((outputPath, idx) => {
                content += `<div style="margin-top: 3px; font-size: 12px;" title="${escapeForHtml(outputPath)}">`;
                content += `<span style="color: var(--text-color-faded);">Output:</span> `;
        ...
        if (rules.length > 0) {
            ...
            rules.forEach((rule, idx) => {
                ...
                if (rulePath) {
                    content += `<div style="margin-top: 3px; font-size: 12px;" title="${escapeForHtml(rulePath)}">`;
        ...
        return `
        <div class="collapsible-section expanded" style="margin-bottom: 8px;">
            <div class="collapsible-header" onclick="toggleSection('ws-diagrams-content')" style="cursor: pointer; padding: 4px 5px; display: flex; align-items: center; user-select: none;">
                <span class="expand-icon">▸</span>
                <span style="font-weight: 600; color: var(--vscode-foreground); font-size: 14px;">Diagrams</span>
            </div>
    ...
    _renderBuildSubsection(instructions, currentAction, botData, jsonIconPath, filesIconPath) {
        let content = '';

        // Open Graph / Open All buttons
        content += `
            <div style="display: flex; align-items: center; gap: 4px; margin-bottom: 8px;">
                <button id="ws-btn-open-graph" onclick="event.stopPropagation(); handleOpenGraph();" style="background: transparent; border: none; padding: 2px; cursor: pointer; transition: opacity 0.15s ease; width: 36px; height: 36px;" onmouseover="this.style.opacity='0.7'" onmouseout="this.style.opacity='1'" title="Open story graph with selected node expanded">
                    <img src="${jsonIconPath}" style="width: 32px; height: 32px; object-fit: contain; display: block;" alt="Graph" />
                </button>
                <button id="ws-btn-open-all" onclick="event.stopPropagation(); handleOpenAll();" style="background: transparent; border: none; padding: 2px; cursor: pointer; transition: opacity 0.15s ease; width: 28px; height: 28px;" onmouseover="this.style.opacity='0.7'" onmouseout="this.style.opacity='1'" title="Open all related files in split editors">
        ...
        if (hasBuildData && currentAction === 'build') {
            ...
            if (Object.keys(schemaData).length > 0) {
                content += `<div style="margin-top: 3px; font-size: 12px; color: var(--text-color-faded);">Story Graph schema loaded</div>`;
            }
        ...
        if (currentAction === 'clarify' && instructions.clarification_data) {
            ...
            if (Array.isArray(clarifyData) && clarifyData.length > 0) {
                content += `<div style="margin-top: 3px; font-size: 12px; color: var(--text-color-faded);">${clarifyData.length} clarification question(s)</div>`;
            }
        }

        return `
        <div class="collapsible-section expanded" style="margin-bottom: 8px;">
            <div class="collapsible-header" onclick="toggleSection('ws-build-content')" style="cursor: pointer; padding: 4px 5px; display: flex; align-items: center; user-select: none;">
                <span class="expand-icon">▸</span>
