---
title: example-tables-use-domain-language
priority: 2
---

## example-tables-use-domain-language

Example tables MUST be grounded in scenario steps AND use domain-rich language. Table columns = nouns from Given/When/Then steps. Use domain terminology
not UI elements. Omit ID columns used purely for linking tables - relationships are expressed via collaboration field and table ordering. Concrete values with domain context
not generic JSON or placeholders. Use source entity data
not aggregated/calculated values - this is the stage where you figure out the real examples.

**DO**

Ground tables in scenario nouns
use domain terminology
connect tables using domain responsibility sentences. Omit implementation IDs. Show source entities
not derived counts.

Omit ID columns - they are implementation concerns. Table relationships are expressed via collaboration field and ordering.
...
Example tables use concept name with collaboration sentences from domain responsibilities
...
Example table name is the concept name
collaboration field stores the connecting responsibility
...
Use domain terminology for column names
...
Use source entity data
not aggregated or calculated values. Show the actual records that produce the outcome
not counts or sums derived from them.
...
Describe actual problems in domain terms with concrete values
...
Show complete expected structures with real values
...
Connect parent-child using domain responsibility as collaboration field
...
Don't use UI elements
flat lookup tables
generic JSON
abstract descriptions
invented terminology
or aggregated/calculated values.

Don't use aggregated or calculated values if THIS scenario is responsible for producing them. If you are computing the result
show the source entities that produce it. You cannot skip the work of figuring out the real examples and just assert the output.
...
Don't use UI element names as columns
...
Don't omit concepts that the scenario explicitly mentions
...
Don't include ID columns for linking tables - IDs are implementation concerns
...
Don't use flat tables with all IDs - model relationships with connected tables
...
Don't invent column names not in domain
...
Don't use raw JSON without explanation
...
Don't use abstract descriptions or placeholders
...
Don't use example name as connecting sentence - use collaboration field instead
...
Don't disconnect examples into lookup tables
